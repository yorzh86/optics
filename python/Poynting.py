...
def Poynting(A_TM, B_TM, z_norm, d, d_norm, kx, eps_O, eps_E, wl,  A_TE=0, B_TE=0):
    #fn returns S_xTE, S_zTE, S_xTM, S_zTM, Tr_TM, E_TMmag
    c0 = 2.99792458E8
    eps0 =8.854187817e-12
    mu0 = 4.0*np.pi/1E7  #what?
...
#    
    ...
    #for TE
    Ey = (A_TE[i]*exp(1j*kzTE(ind)*(z-z_0[i]))+B_TE[i]*np.exp(-1j*kzTE[i]*(z-z_0[i])))*np.exp(1j*kx)
    ...
    S_xTE = 0.5*real(Ey*Hz_conj)   # return this
    S_zTE = -0.5*real(Ey*Hx_conj)  # return this
